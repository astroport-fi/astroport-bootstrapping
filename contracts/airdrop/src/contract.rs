...
pub fn instantiate( deps: DepsMut, _env: Env, _info: MessageInfo, msg: InstantiateMsg) -> StdResult<Response> {
    ...
    let config = Config {
        ...
        astro_token_address: deps.api.addr_validate(&msg.astro_token_address.unwrap_or(Addr::unchecked("").to_string() ))?,
...
pub fn execute( deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg)  -> Result<Response, StdError> {
    match msg {
        ...
        ExecuteMsg::TransferAstroTokens { 
            recepient, 
            amount 
        }  => handle_transfer_astro(deps, env,  info, recepient, amount),
    }
}
...
pub fn handle_terra_user_claim( 
    ...
    let transfer_msg = build_send_cw20_token_msg(user_account.clone(), config.astro_token_address.to_string(), claim_amount.into())?;
...
pub fn handle_evm_user_claim( 
    ...
    // COSMOS MSG :: CLAIM AMOUNT TRANSFERRED
    let transfer_msg = build_send_cw20_token_msg(recepient_account.clone(), config.astro_token_address.to_string(), claim_amount.into())?;
...
pub fn handle_transfer_astro( 
    ...
    .add_message(transfer_msg)        
    .add_attributes(vec![
        attr("action", "Airdrop::ExecuteMsg::TransferAstroTokens"),
...
mod tests {
    ...
    use crate::msg::ExecuteMsg::{UpdateConfig, ClaimByTerraUser , ClaimByEvmUser, TransferAstroTokens};
    ...
    fn test_transfer_astro_tokens() {
        ...
        // *** Test Transfer Msg ***
        let transfer_msg = TransferAstroTokens {
