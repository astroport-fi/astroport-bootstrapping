...
pub fn instantiate(
    ...
    let state = State {
        ...
        total_astro_unclaimed: Uint256::zero(),
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::InitializePool { 
         ...
         ExecuteMsg::WithdrawFromLockup { duration, amount } => {
            handle_withdraw_from_lockup(deps, _env, info, duration, amount)
        },

        ExecuteMsg::DelegateAstroToAuction { amount } => {
            handle_delegate_astro_to_auction(deps, _env, info, amount)
        },
        ForceUnlockPosition { lp_token_address, duration } =>  handle_force_unlock_position(deps, _env, info, lp_token_address, duration)
...
pub fn receive_cw20(
    ...
    let lp_token_addr = info.sender.as_str();
...
pub fn handle_initialize_pool(
    ...
    if is_str_present_in_vec(&state.supported_lp_tokens, lp_token_addr.clone()) {
        return Err(StdError::generic_err("LP Token already supported"));
    }    
    ...
    // CHECK ::: Pool should not exist 
    if pool_info.lp_token_addr != zero_address() {
        return Err(StdError::generic_err("Pool already initialized"));
    }
    ...
    match pool_type { 
        PoolType::Terraswap {} => { pool_info.pool_type = "terraswap" },
        PoolType::Astroport {} => { pool_info.pool_type = "astroport" },
        None  => {
...
pub fn handle_increase_lockup(
    ...
    if !is_str_present_in_vec(&user_info, lockup_id.clone()) {
...
pub fn handle_withdraw_from_lockup(
    ...
    if  withdrawals_status.max_withdrawal_percent <= Decimal256::from_ratio(50u128,100u128) {
...
pub fn handle_delegate_astro_to_auction(
    ...
    if user_info.lockup_positions.len() == 0 {
        return Err(StdError::generic_err("User has no valid lockup positions"));
    }
...
fn is_str_present_in_vec(user_info: &UserInfo, lockup_id: String) -> bool {
