...
use astroport_periphery::helpers::{cw20_get_balance, zero_address,build_approve_cw20_msg, build_transfer_cw20_token_msg,
    build_send_cw20_token_msg,build_transfer_cw20_from_user_msg, option_string_to_addr, is_str_present_in_vec } ;
use astroport_periphery::lockdrop::{ 
    WithdrawalStatus, Cw20HookMsg, CallbackMsg, ConfigResponse, ExecuteMsg, StateResponse,PoolResponse, InstantiateMsg,
    LockUpInfoResponse, UserInfoResponse, UpdateConfigMsg, QueryMsg
};
...
use astroport_periphery::asset::{Asset, AssetInfo, NativeAsset, Cw20Asset, LiquidityPool} ;
use astroport_periphery::tax::{deduct_tax};

use crate::state::{self, ASSET_POOLS, CONFIG, Config, LOCKUP_INFO, LockupInfo, PoolInfo, STATE, State, USER_INFO, UserInfo};
...
pub fn execute(
    ...
    match msg {
        ...
        } => handle_migrate_liquidity(deps, _env, info, pool_identifer, astroport_pool_address, astroport_lp_address),

         ExecuteMsg::StakeLpTokens { pool_identifer } => handle_stake_lp_tokens(deps, _env,  info, pool_identifer),
         ExecuteMsg::EnableClaims {} => handle_enable_claims(deps,  info),
  
         ExecuteMsg::WithdrawFromLockup { pool_identifer, duration, amount } => {
        ...
        ExecuteMsg::ClaimRewardsForLockup { pool_identifer, duration } => handle_claim_rewards_for_lockup(deps, _env, info, pool_identifer, duration),

        ExecuteMsg::UnlockPosition { pool_identifer, duration } => handle_unlock_position(deps, _env, info, pool_identifer, duration),
        ExecuteMsg::ForceUnlockPosition { pool_identifer, duration } => handle_force_unlock_position(deps, _env, info, pool_identifer, duration),
...
fn _handle_callback(
    ...
    match msg {
        ...
        } => update_pool_on_dual_rewards_claim(deps, env, pool_identifer, prev_astro_balance, prev_dual_reward_balance),
        CallbackMsg::WithdrawUserLockupRewardsCallback { 
            ...
            withdraw_lp_stake,
            force_unlock
        } => callback_withdraw_user_rewards_for_lockup_optional_withdraw(deps, env, user_address, pool_identifer, duration,withdraw_lp_stake, force_unlock),
        CallbackMsg::WithdrawLiquidityFromTerraswapCallback {
            ...
            cw20_asset_balance,
            astroport_pool
        } => callback_deposit_liquidity_in_astroport(deps, env,info, pool_identifer, native_asset, native_asset_balance, cw20_asset, cw20_asset_balance, astroport_pool),
        CallbackMsg::UpdateStateLiquidityMigrationCallback {
            pool_identifer,
            astroport_pool,
            astroport_lp_balance
        } => callback_update_pool_state_after_migration(deps, env,info, pool_identifer, astroport_pool, astroport_lp_balance)
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::LockUpInfo { user_address, pool_identifier, duration } => {
            to_binary(&query_lockup_info(deps, user_address, pool_identifier, duration)?)
        }
...
pub fn handle_initialize_pool(
    ...
    if is_str_present_in_vec(state.supported_pairs_list.clone(), terraswap_pool.lp_token_addr.clone().to_string()) {
    ...
    let mut pool_info = PoolInfo {
        ...
        astroport_pair: LiquidityPool { lp_token_addr: zero_address(),  pair_addr: zero_address(), amount: Uint256::zero() } ,
        cw20_asset: Cw20Asset { contract_addr: "".to_string() },
        native_asset: NativeAsset { denom: "uuusd".to_string() }, 
    ...
    let pool_response: terraswap::pair::PoolResponse = deps.querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
    ...
    state.supported_pairs_list.push( terraswap_pool.lp_token_addr.clone().to_string() );
    ASSET_POOLS.save(deps.storage, &terraswap_pool.lp_token_addr.as_bytes(), &pool_info)?;
    STATE.save(deps.storage,  &state)?;
    ...
    Ok(Response::new().add_attributes(vec![
        ...
        ("cw20_asset_addr", pool_info.cw20_asset.contract_addr.to_string().as_str()),
...
pub fn handle_migrate_liquidity(
    ...
    let withdraw_liquidity_msg = build_send_cw20_token_msg( pool_info.terraswap_pair.pair_addr.to_string(), pool_info.terraswap_pair.lp_token_addr.to_string(), pool_info.terraswap_pair.amount.into(), withdraw_msg_ )?;

    // QUERY :: Get current Asset and uusd balances to calculate how much liquidity was withdrawn from the terraswap pool
    let asset_balance = cw20_get_balance(&deps.querier, deps.api.addr_validate(&pool_info.cw20_asset.contract_addr)? ,  env.contract.address.clone()  )?;
    let native_balance_response = deps.querier.query_balance(env.contract.address.clone(), pool_info.native_asset.denom.to_string()  )?;
    ...
    let update_state_msg = CallbackMsg::WithdrawLiquidityFromTerraswapCallback { 
                                                    ...
                                                    native_asset: NativeAsset { denom: pool_info.native_asset.denom.to_string()  }  , 
                                                    native_asset_balance: native_balance_response.amount,                                                    
                                                    cw20_asset: Cw20Asset{ contract_addr: pool_info.cw20_asset.contract_addr.to_string()  }  , 
                                                    cw20_asset_balance: asset_balance,
                                                    astroport_pool: LiquidityPool { lp_token_addr: deps.api.addr_validate(&astroport_lp_address)?, pair_addr:  deps.api.addr_validate(&astroport_pool_address)?, amount: Uint256::zero() } ,
...
pub fn handle_increase_lockup(
    ...
    let lockup_id = user_address.to_string().clone() + &pool_identifer.clone() + &duration.to_string();
...
pub fn handle_withdraw_from_lockup(
    ...
    let withdrawals_status = calculate_max_withdrawal_percent_allowed(env.block.time.seconds(), &config);
    ...
    let send_cw20_msg = build_transfer_cw20_token_msg(user_address.clone(), pool_info.terraswap_pair.lp_token_addr.to_string(), amount.into() )?;
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            ...
            ("user", &user_address.to_string()),
            ("lp_token_addr", &pool_info.terraswap_pair.lp_token_addr.to_string()),
            ("duration", duration.to_string().as_str()),
            ("amount", amount.to_string().as_str()),
        ]))
}
...
pub fn handle_delegate_astro_to_auction(
    ...
    if user_info.total_astro_rewards == Uint256::zero() {
        ...
        for lockup_id in &mut user_info.lockup_positions {
            ...
            let pool_info = ASSET_POOLS.load(deps.storage, &lockup_info.pool_identifier.as_bytes() ).unwrap();
            ...
            if pool_info.is_migrated && !lockup_info.is_migrated {
                lockup_info.astroport_lp_units = calculate_lockup_balance_post_migration(lockup_info.lp_units_locked, pool_info.terraswap_pair.amount, pool_info.astroport_pair.amount );
            ...
            lockup_info.astro_rewards = calculate_astro_incentives_for_lockup(weighted_lockup_balance, pool_info.weighted_amount, pool_info.incentives_percent, config.lockdrop_incentives );
...
//
pub fn calculate_astro_incentives_for_lockup(lockup_weighted_balance: Uint256, total_weighted_amount: Uint256, incentives_percent:Decimal256, total_lockdrop_incentives: Uint256 ) -> Uint256 {
    let total_pool_incentives = incentives_percent * total_lockdrop_incentives;
    let percent_weight_of_total = Decimal256::from_ratio(lockup_weighted_balance, total_weighted_amount);
...
pub fn handle_stake_lp_tokens(
    ...
    if is_str_present_in_vec(state.supported_pairs_list.clone(), pool_identifer.clone().to_string()) {
        return Err(StdError::generic_err("Already supported"));
    }     
    let mut pool_info = ASSET_POOLS.load(deps.storage, &pool_identifer.clone().as_bytes() )?;
    ...
    if !pool_info.is_migrated {
        return Err(StdError::generic_err("Only Astroport LP Tokens can be staked with generator"));
    } 

    //  COSMOSMSG :: If LP Tokens are migrated, used LP after migration balance else use LP before migration balance
    let stake_lp_msg = build_stake_with_generator_msg( config.generator_address.to_string().clone(),  pool_info.astroport_pair.lp_token_addr.clone(), pool_info.astroport_pair.amount.into() )?;
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            ...
            ("staked_amount", pool_info.astroport_pair.amount.to_string().as_str()),
    ]))
}
...
pub fn handle_claim_rewards_for_lockup(
    ...
    if unclaimed_rewards_response.pending > Uint128::zero()  {
        ...
        cosmos_msgs.push(build_claim_dual_rewards(_env.contract.address.clone(), pool_info.astroport_pair.pair_addr.clone(), config.generator_address.clone())?);
        let astro_balance = cw20_get_balance(&deps.querier, config.astro_token_address,  _env.contract.address.clone()  )?;
        let dual_reward_balance =  cw20_get_balance(&deps.querier,  deps.api.addr_validate(&pool_info.cw20_asset.contract_addr)?,  _env.contract.address.clone()  )?;        
        let update_pool_state_msg = CallbackMsg::UpdatePoolOnDualRewardsClaim { pool_identifer: pool_identifer.clone(), 
...
pub fn handle_unlock_position(
    ...
    let lockup_id = user_address.to_string().clone() + &pool_identifer.to_string().clone() + &duration.to_string();
    ...
    if unclaimed_rewards_response.pending > Uint128::zero() { 
        // QUERY :: Current ASTRO & DUAL Reward Token Balance
        // -->add CallbackMsg::UpdatePoolOnDualRewardsClaim{} msg to the cosmos msg array   
        cosmos_msgs.push(build_claim_dual_rewards(env.contract.address.clone(), pool_info.astroport_pair.pair_addr.clone(), config.generator_address.clone())?);
        let astro_balance = cw20_get_balance(&deps.querier, config.astro_token_address,  env.contract.address.clone()  )?;
        let dual_reward_balance = cw20_get_balance(&deps.querier,  deps.api.addr_validate(&pool_info.cw20_asset.contract_addr)?,  env.contract.address.clone()  )?;        
        let update_state_msg = CallbackMsg::UpdatePoolOnDualRewardsClaim { pool_identifer: pool_identifer.clone(), 
                                                                                    prev_astro_balance: astro_balance.into(),
                                                                                    prev_dual_reward_balance: dual_reward_balance.into(),
                                                                                }.to_cosmos_msg(&env.contract.address)?;
...
pub fn update_pool_on_dual_rewards_claim(
    ...
    let cur_dual_reward_balance = cw20_get_balance(&deps.querier, deps.api.addr_validate(&pool_info.cw20_asset.contract_addr.clone())?, env.contract.address.clone() )?;
...
pub fn callback_withdraw_user_rewards_for_lockup_optional_withdraw(
    ...
    if pending_dual_rewards > Uint256::zero() {
        cosmos_msgs.push( build_transfer_cw20_token_msg(user_address.clone(), pool_info.cw20_asset.contract_addr.clone().to_string(), pending_dual_rewards.into()  )? );
    }
    ...
    if withdraw_lp_stake {
        ...
        if pool_info.is_staked {
            let unstake_lp_msg = build_unstake_from_generator_msg( config.generator_address.clone().to_string(),  pool_info.astroport_pair.lp_token_addr.clone(), lockup_info.astroport_lp_units)?;
            cosmos_msgs.push(unstake_lp_msg);
        }
        // COSMOSMSG :: Returns LP units locked by the user in the current lockup position
        let transfer_lp_msg = build_transfer_cw20_token_msg( user_address.clone(),  pool_info.astroport_pair.lp_token_addr.clone().to_string(), lockup_info.astroport_lp_units.into() )?;
        ...
        // remove_lockup_pos_from_user_info(&mut user_info, lockup_id.clone());
        pool_info.astroport_pair.amount = pool_info.astroport_pair.amount - lockup_info.lp_units_locked;        
...
pub fn callback_deposit_liquidity_in_astroport(    
    ...
    // QUERY :: Get current Asset and uusd balances to calculate how much liquidity was withdrawn from the terraswap pool
    let astroport_lp_balance = cw20_get_balance(&deps.querier, astroport_pool.lp_token_addr.clone(),  env.contract.address.clone()  )?;
    let asset_balance = cw20_get_balance(&deps.querier, deps.api.addr_validate(&pool_info.cw20_asset.contract_addr)?,  env.contract.address.clone()  )?;
    let native_balance_response = deps.querier.query_balance(env.contract.address.clone(), pool_info.native_asset.denom.to_string()  )?;
    ...
    let cw20_asset_ =  astroport::asset::Asset { 
        info: astroport::asset::AssetInfo::Token { contract_addr: deps.api.addr_validate(&cw20_asset.contract_addr.to_string())? },
        amount: asset_balance_withdrawn.into()
    };
    let native_asset_ =  astroport::asset::Asset { 
        info: astroport::asset::AssetInfo::NativeToken { denom: native_asset.denom.clone() },
    ...
    let approve_cw20_msg = build_approve_cw20_msg( cw20_asset.contract_addr, astroport_pool.pair_addr.to_string(), asset_balance_withdrawn.into() )?;
    let add_liquidity_msg = build_provide_liquidity_to_lp_pool_msg( deps.as_ref(), assets_, astroport_pool.pair_addr.clone() , native_asset.denom, native_balance_withdrawn)?;
    let update_state_msg = CallbackMsg::UpdateStateLiquidityMigrationCallback {    pool_identifer: pool_identifer.clone(),
...
fn compute_lockup_position_accrued_astro_rewards(pool_info: &PoolInfo, lockup_info: &mut LockupInfo) -> Uint256 {
    let pending_astro_rewards = (lockup_info.astroport_lp_units * pool_info.astro_global_reward_index)  - (lockup_info.astroport_lp_units * lockup_info.astro_reward_index);
...
fn compute_lockup_position_accrued_dual_rewards(pool_info: &PoolInfo, lockup_info: &mut LockupInfo) -> Uint256 {
    let pending_dual_rewards = (lockup_info.astroport_lp_units * pool_info.asset_global_reward_index)  - (lockup_info.astroport_lp_units * lockup_info.dual_reward_index);
...
fn calculate_max_withdrawal_percent_allowed(current_timestamp: u64, config: &Config) -> WithdrawalStatus {
...
fn update_pool_reward_indexes(pool_info: &mut PoolInfo, astro_accured: Uint256, dual_rewards_accured: Uint256)  {
    if !pool_info.is_staked {
        return;
    }
    let astro_rewards_index_increment = Decimal256::from_ratio( astro_accured, pool_info.astroport_pair.amount  );
    pool_info.astro_global_reward_index = pool_info.astro_global_reward_index + astro_rewards_index_increment;

    let dual_rewards_index_increment = Decimal256::from_ratio( dual_rewards_accured, pool_info.astroport_pair.amount  );
...
//
pub fn calculate_lockup_balance_post_migration(user_lp_units: Uint256, total_terraswap_lp_locked:Uint256, total_astroport_lp_locked:Uint256) -> Uint256 {
...
//
fn build_provide_liquidity_to_lp_pool_msg( deps: Deps, assets_: [astroport::asset::Asset;2], astroport_pool: Addr, native_denom:String, native_amount: Uint128) -> StdResult<CosmosMsg> {
